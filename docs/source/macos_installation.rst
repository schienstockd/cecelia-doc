...
5. Install package dependencies.
  ...
  * When prompted during the process, Select “1” to restore the project from the lockfile.
  ...
  .. attention::
    Please make sure that this command installs the required packages, that is, there should be a bunch of packages including some from Github and Bioconductor. If that is not the case, check that the `lock-file` has packages listed and repeat the previous step with `renv::init()`.
