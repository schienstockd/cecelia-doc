...
5. Install package dependencies.
  ...
  * `THIS STEP WILL INSTALL ALL REQUIRED R PACKAGES. THERE IS NO NEED TO MANUALLY INSTALL PACKAGES. IF YOU ARE MISSING PACKAGES WHEN STARTING THE APP, THEN YOU NEED TO COME BACK TO THIS STEP.`
  ...
  .. attention::
    Make sure that this command installs ALL required packages, that is, there should be a long list of packages including some from Github and Bioconductor. If that is not the case, check that the `lock-file` has packages listed, download the file again if needed, and repeat the previous step with ``renv::init()``. We had cases where the `lock-file` was reset and not all packages installed.
